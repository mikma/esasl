...
static int start(char *buf, int index, ei_x_buff *presult)
{
    ...
    /* {new, {Mode, Mech, Service, Host_name}} */
	...
	/* TODO, flush input */
	if (arity != 4) ENCODE_ERROR("badarg");
...
static int step(char *buf, int index, ei_x_buff *presult)
{
	...
	if (arity != 2) ENCODE_ERROR("badarg");
...
static int property_set(char *buf, int index, ei_x_buff *presult)
{
	...
	if (arity != 3) ENCODE_ERROR("badarg");
...
static int mechlist(char *buf, int index, ei_x_buff *presult)
{
    ei_x_buff result = *presult;
    /* {mechlist, client|server} */
	...
	if (strcmp(mode, "server") == 0) {
	    OK(gsasl_server_mechlist(g_ctx, &list));
	} else if (strcmp(mode, "client") == 0) {
	    OK(gsasl_client_mechlist(g_ctx, &list));
	} else {
	    ENCODE_ERROR("invalid_mode");
	}

	if (ei_x_encode_atom(&result, "ok") ||
	    ei_x_encode_string(&result, list))
	    return 46;
	if (list) free(list);
    }while(0);
...
/*     fprintf(stderr, "No more commands, exiting\r\n"); */
